...
import { FORMULES } from '@/content/stages'
import type { StageConfig, StageId } from '@/content/stages'

const CARD_DETAILS: Record<StageId, { headline: string; sub: string; note: string }> = {
  'juillet-2026': {
    headline: 'Lundi au jeudi',
    sub: '8 jours',
    note: 'Pas de tarif semaine (vendredis indisponibles)',
  },
  'aout-2026': {
    headline: 'Lundi au vendredi',
    sub: '10 jours',
...
const StagesHighlight = ({ stages }: { stages: StageConfig[] }) => {
  const stageCards = stages.map((stage) => ({ stage, ...CARD_DETAILS[stage.id] }))
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-cheddar font-bold text-gray-900 mb-4 sm:mb-6">
            {stageCards.length > 1 ? 'Prochains stages' : 'Prochain stage'}
          </h2>
          <p className="text-lg sm:text-xl font-montserrat text-gray-600 max-w-3xl mx-auto px-4">
            Stage de parkour pendant les vacances scolaires. Choisissez vos dates et votre formule, pour tous à partir de 6 ans.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className={`grid grid-cols-1 gap-4 sm:gap-6 mb-12 mx-auto ${stageCards.length > 1 ? 'md:grid-cols-2 max-w-4xl' : 'max-w-md'}`}
        >
...
export default StagesHighlight
